...
public class SocketChannelWriter {
	private static final String TAG = "SocketChannelWriter";
	...
	//write from VPN server to final destination
	...
	//write data from vpnServer to final destination server (data is received from app's on the device  app -> vpn server -> final destination server) SENT WHEN PSH FLAG IS ACTIVE?
	private void writePendingData(Session session) throws IOException {
		...
		session.addBytesSent(buffer.remaining());
