...
public class SocketNIODataService implements Runnable {
	...
	public void registerSession(Session session) throws ClosedChannelException {
		...
		Log.i(TAG, "Registering new session: " + PacketUtil.intToIPAddress(session.getSourceIp())+":"+ session.getSourcePort() + " -> "+ PacketUtil.intToIPAddress(session.getDestIp()) +":"+ session.getDestPort() + " " + session);
